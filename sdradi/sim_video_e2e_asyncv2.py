            ...
            if mac_fec_parity and int(mac_fec_parity) > 0:
                ...
                if frame_info:
                    ...
                    for p_idx in range(parity_count):
                        seed = (int(mac_fec_seed) + int(frame_info['frame_id']) * 1315423911 + p_idx * 2654435761) & 0xFFFFFFFF
                        rng = np.random.RandomState(seed)
                        coeffs = rng.randint(0, 2, size=data_total).astype(np.uint8)
                        if not np.any(coeffs):
                            coeffs[rng.randint(0, data_total)] = 1
                        parity_payload = np.zeros(payload_size, dtype=np.uint8)
                        for i in range(data_total):
                            if coeffs[i]:
                                parity_payload ^= np.frombuffer(data_payloads[i], dtype=np.uint8)
...
def run_rx_node(output_file, config_file="sdr_tuned_config.json", width=320, height=240, fec_cfg=None, sync_threshold=40.0, chunk_samples=0, frame_timeout=5.0, log_interval=2.0, store_timeout=15.0, mac_parity=False, diag_level="basic", mac_parity_groups=1, header_repeat_bytes=0, mac_fec_parity=0, mac_fec_seed=1):
    ...
    if fec_cfg.enabled:
        ...
        elif fec_cfg.fec_type == FECType.CONVOLUTIONAL:
            expected_fec_bits = 2 * (expected_packet_bits + fec_cfg.constraint_length - 1)
        else:
    ...
    out = cv2.VideoWriter(output_file, cv2.VideoWriter_fourcc(*'MJPG'), 10.0, (width, height))
    ...
    try:
        while not stop_event.is_set():
            ...
            while True:
                ...
                for shift in range(0, scan_max + 1):
                    ...
                    if candidate_info:
                        if (
                            candidate_info['width'] != width
                            or candidate_info['height'] != height
                ...
                def build_coeffs(total_data, parity_idx, frame_id):
                    seed = (int(mac_fec_seed) + int(frame_id) * 1315423911 + parity_idx * 2654435761) & 0xFFFFFFFF
                    rng = np.random.RandomState(seed)
                    coeffs = rng.randint(0, 2, size=total_data).astype(np.uint8)
                    if not np.any(coeffs):
                        coeffs[rng.randint(0, total_data)] = 1
                    return coeffs
                ...
                def solve_missing(missing_indices, parity_indices, data_total, frame_id, payloads, payload_size):
                    ...
                    for row in range(m):
                        ...
                        for i in range(data_total):
                            if coeffs[i]:
                                if i in payloads:
                                    eq ^= np.frombuffer(payloads[i], dtype=np.uint8)
            ...
            for k in expired:
                ...
                del frame_buffer[k]
            with tx_packet_lock:
                expired_pkts = [k for k,v in tx_packet_store.items() if now - v['ts'] > store_timeout]
                for k in expired_pkts:
                    del tx_packet_store[k]
            if now - last_log >= log_interval:
                with stats_lock:
                    tx_pkts = stats['tx_packets']
                ...
                if diag_level in ("basic", "phy", "mac", "all"):
                    print(f"[RX] Frames: {frames_total}/{tx_frames}, Packets: {rx_pkts}/{tx_pkts}, CRC: {crc_total}, FER: {fer:.3f}, Success: {pkt_success:.3f}")
        ...
        if diag_level in ("basic", "phy", "mac", "all"):
            ...
            print(f"[RX] Frames: TX={tx_frames}, RX_OK={frames_total}, Started={frames_started}, Expired={frames_expired}, FER={fer:.3f}")
        if diag_level in ("phy", "all"):
            print(f"[RX] PreBER: {pre_ber:.4e}, PostBER: {post_ber:.4e}")
            print(f"[RX] Sync: {sync_successes}/{sync_attempts}, Incomplete: {sync_incomplete}, SyncRate: {sync_rate:.3f}")
        if diag_level in ("mac", "all"):
...
def main():
    ...
    parser.add_argument("--sync-threshold", type=float, default=40.0, help="Sync correlation threshold")
    ...
    parser.add_argument("--sleep-factor", type=float, default=0.9, help="TX pacing factor")
    parser.add_argument("--max-frames", type=int, default=120, help="Max frames to send")
    parser.add_argument("--pkt-repeats", type=int, default=1, help="TX repeat count per packet")
    parser.add_argument("--frame-timeout", type=float, default=5.0, help="RX frame buffer timeout (seconds)")
    ...
    parser.add_argument("--store-timeout", type=float, default=15.0, help="TX packet store timeout (seconds)")
    ...
    parser.add_argument("--mac-fec-parity", type=int, default=0, help="Cross-packet parity count per frame")
    ...
    parser.add_argument("--header-repeat-bytes", type=int, default=0, help="Repeat header bytes in packet body for alignment slack")
    ...
    rx_thread = threading.Thread(
        target=run_rx_node,
        args=(args.output, config_file, args.width, args.height, fec_cfg, args.sync_threshold, args.chunk_samples, frame_timeout, args.log_interval, store_timeout, args.mac_parity, args.diag_level, args.mac_parity_groups, args.header_repeat_bytes, args.mac_fec_parity, args.mac_fec_seed)
    )
    tx_thread = threading.Thread(
        target=run_tx_node,
        args=(video_path, config_file, allow_synthetic, args.width, args.height, args.quality, fec_cfg, args.gap_samples, args.sleep_factor, args.max_frames, pkt_repeats, args.log_interval, store_timeout, args.mac_parity, args.diag_level, args.mac_parity_groups, args.header_repeat_bytes, args.repeat_strategy, args.mac_fec_parity, args.mac_fec_seed)
    )
