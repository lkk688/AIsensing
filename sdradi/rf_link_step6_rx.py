...
def resolve_sc_cfo_alias(rx, fs, sc_cfo_hz, tone_freq=100e3, L=N_FFT // 2, num_aliases=4):
    ...
    When the hardware LO offset is close to the tone frequency the tone lands
    near DC, getting zeroed by DC-removal or the standard near-DC mask.
    This function sweeps num_aliases alias candidates on each side of the SC
    estimate, applies each as a trial CFO correction, and picks whichever one
    places the pilot tone at tone_freq with the most spectral power.
...
def demod_one_capture(
    ...
    # Pluto hardware LO offsets of ±100+ kHz shift the 100 kHz pilot tone
    # near DC where the standard masked estimator fails silently.
