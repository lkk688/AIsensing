...
def demod_one_capture(
    ...
    # )
    # ---- 0) Schmidl-Cox on *raw* rx (no CFO applied yet) ----
    sc_detected0, sc_idx0, sc_peak0, sc_cfo_norm0, sc_M0 = detect_stf_autocorr(
        rx, half_period=N_FFT // 2, threshold=sc_threshold
    )
    cfo_sc_hz = float(sc_cfo_norm0 * fs)

    # ---- 1) Tone CFO (diagnostic / optional) ----
    seg_len = 20000
    best_cfo_tone = 0.0
    best_snr_lin = 0.0
    for seg_start in range(0, min(len(rx) - seg_len, 80000), 5000):
    ...
    tone_snr_db = 10.0 * np.log10(best_snr_lin + 1e-12)
    cfo_tone_hz = float(best_cfo_tone)

    # ---- 2) Choose CFO ----
    # args is from outer scope (main). We'll pass these knobs into demod_one_capture.
    # So add these parameters to demod_one_capture signature: cfo_mode, tone_snr_db_th, tone_cfo_abs_max_hz, sc_threshold
    use_tone = (tone_snr_db >= tone_snr_db_th) and (abs(cfo_tone_hz) <= tone_cfo_abs_max_hz)
    ...
    # Apply CFO correction once
    rx_cfo = apply_cfo_correction(rx, cfo_hz, fs)

    # ---- 3) Now do Schmidl-Cox + xcorr on CFO-corrected buffer ----
    sc_detected, sc_idx, sc_peak, sc_cfo_norm, sc_M = detect_stf_autocorr(
        rx_cfo, half_period=N_FFT // 2, threshold=sc_threshold
    )
    ...
    result = {
        ...
        'cfo_tone_hz': float(cfo_tone_hz),
        'cfo_sc_hz_raw': float(cfo_sc_hz),
        'stf_idx': stf_idx,
        'stf_peak': stf_peak,
        'stf_method': method,
        'sc_peak': sc_peak,
        'sc_cfo_norm': sc_cfo_norm,   # after correction, mainly diagnostic
        'xc_peak': xc_peak,
        'sc_metric': sc_M,
        'xc_metric': xc_corr,
        'xc_idx': xc_idx,
...
def main():
    ...
    ap.add_argument("--cfo_mode", default="sc", choices=["sc", "tone", "auto"],
                help="CFO source: sc=SchmidlCox (recommended), tone=pilot tone, auto=gated tone else sc")
    ap.add_argument("--tone_snr_db_th", type=float, default=12.0,
                help="(auto) only use tone CFO if tone SNR(dB) >= this")
    ap.add_argument("--tone_cfo_abs_max_hz", type=float, default=20000.0,
                help="(auto) only use tone CFO if |cfo_tone| <= this")
    ap.add_argument("--sc_threshold", type=float, default=0.1,
                help="Schmidl-Cox detect threshold")
    ...
    for t in range(num_captures):
        ...
        r = demod_one_capture(
            ...
            tone_snr_db_th=args.tone_snr_db_th,
            tone_cfo_abs_max_hz=args.tone_cfo_abs_max_hz,
