...
def main():
    ...
    sdr_cfg.device = 'pluto' # Force pluto for loopback
    ...
    if args.loopback != 'disabled':
        if args.loopback == 'digital':
            print("[SDR] Overriding config for Digital Loopback (FS=3M, BW=3M, LO=915M)")
            sdr_cfg.fs = 3000000
            sdr_cfg.bandwidth = 3000000
            sdr_cfg.fc = 915000000
            if args.tx_gain == 0 and args.rx_gain == 40:
                print("[SDR] Digital Loopback: Preserving configured/default gains (BIST independent)")
                # sdr_cfg.tx_gain = 0
                # sdr_cfg.rx_gain = 70
                pass
            else:
            ...
            print("[SDR] Overriding config for RF Loopback (FS=3M, BW=10M, LO=2405M)")
            sdr_cfg.fs = 3000000
            sdr_cfg.bandwidth = 10000000
            sdr_cfg.fc = 2405000000
            if args.tx_gain == 0 and args.rx_gain == 40:
    ...
    # Conflict: This sets loopback on a temp context BEFORE the main SDR is initialized.
    # The initialization likely resets it. Removing to rely on post-init configuration.
    # if args.loopback == 'digital':
    ...
    # --- APPLY GAINS HERE (After Initialization) ---
    try:
        print("[SDR] Applying Gains...")
        ...
        if args.loopback == 'rf':
            # RF Loopback settings
            adi_dev.tx_lo = int(2405e6)
            adi_dev.rx_lo = int(2405e6)
            # Increase Bandwidth >> Sample Rate (3M) to absolutely avoid filter roll-off
            # Try 10 MHz
            adi_dev.tx_rf_bandwidth = 10000000  
            adi_dev.rx_rf_bandwidth = 10000000
            adi_dev.tx_hardwaregain_chan0 = args.tx_gain 
            adi_dev.gain_control_mode_chan0 = 'manual'
            adi_dev.rx_hardwaregain_chan0 = args.rx_gain 
            print(f"Configured for RF loopback: TX/RX gain={args.tx_gain}/{args.rx_gain} dB, BW=10MHz")
            
            # Readback removed as it caused hang
    ...
    if args.loopback != 'disabled':
        ...
        if args.loopback == 'digital':
            # Force debug_attrs usage as adi_dev.loopback property is often missing/no-op on Pluto
            try:
                adi_dev.ctx.find_device('ad9361-phy').debug_attrs['loopback'].value = '1'
                print("[SDR] Digital Loopback Enabled via debug_attrs")
            except Exception as e:
                print(f"[SDR] Failed to set loopback: {e}")
            
            # Remove conflicting helper call
            # set_loopback_mode(args.ip, 1)
        elif args.loopback == 'rf':
            set_loopback_mode(args.ip, 0)
    ...
    elif fec_cfg.enabled and (fec_cfg.fec_type == FECType.CONVOLUTIONAL or (fec_cfg.fec_type == FECType.LDPC and not hasattr(link.fec_codec, 'n'))):
    ...
    try:
        print(f"[SDR] RX buffer actual: {adi_dev.rx_buffer_size}")
    except Exception:
        pass
    ...
    try:
        for frame_idx in range(args.max_frames):
            ...
            for pkt_idx, (pkt_bytes, pkt_i) in enumerate(packets):
                ...
                if args.loopback == 'digital':
                    scale_factor = 2**14 * 0.25
                else:
                    scale_factor = 2**14 * 0.125
                ...
                if args.verbose:
                    ...
                    # Readback actual gains to ensure they stuck
                    try:
                        act_tx = adi_dev.tx_hardwaregain_chan0
                        act_rx = adi_dev.rx_hardwaregain_chan0
                        print(f"    [Gain Debug] Set: {args.tx_gain}/{args.rx_gain}, Actual: {act_tx}/{act_rx}")
                    except: pass
                ...
                if args.mode == 'cyclic':
                    ...
                    try:
                        ...
                        tx_norm = tx_signal_full / np.max(np.abs(tx_signal_full)) if np.max(np.abs(tx_signal_full)) > 0 else tx_signal_full
                        
                        for rx_attempt in range(max_rx_attempts):
                            ...
                            if len(rx_stream) < len(tx_signal_full) + len(tx_signal):
                                continue
                            
                            max_rx = np.max(np.abs(rx_stream))
                            if args.verbose:
                                 print(f"    [RX Raw] Max Level: {max_rx:.2f}")
                            rx_stream_norm = rx_stream / max_rx if max_rx > 0 else rx_stream
                            
                            corr = np.abs(np.correlate(rx_stream_norm, tx_norm, mode='valid'))
                            peak_idx = np.argmax(corr)
                            payload_start = peak_idx + len(preamble)
                            if payload_start + len(tx_signal) > len(rx_stream_norm):
                                continue
                            
                            payload = rx_stream_norm[payload_start:payload_start + len(tx_signal)]
                            ...
                            try:
                                ...
                                if len(rx_bits) >= len(bits):
                                    ...
                                    if not crc_ok:
                                        payload_c = np.conj(payload)
                                        rx_fec_bits_c, demod_metrics_c = link.transceiver.demodulate(payload_c)
                                        if args.verbose and demod_metrics_c:
                                            snr_est = demod_metrics_c.get('snr_est_db', 0)
                                            ch_gain = demod_metrics_c.get('channel_gain_db', 0)
                                            print(f"    [Demod*] SNR={snr_est:.1f}dB, ChGain={ch_gain:.1f}dB, Bits={len(rx_fec_bits_c)}")
                                        rx_bits_c = link.fec_codec.decode(rx_fec_bits_c)
                                        if len(rx_bits_c) >= len(bits):
                                            rx_bits_c = rx_bits_c[:len(bits)]
                                            rx_bytes_c = np.packbits(rx_bits_c).tobytes()
                                            info_c = link.video_codec.parse_packet_header(rx_bytes_c)
                                            if info_c:
                                                calc_crc_c = (zlib.crc32(info_c['payload']) & 0xFFFFFFFF)
                                                if calc_crc_c == info_c['crc']:
                                                    info = info_c
                                                    rx_bits = rx_bits_c
                                                    crc_ok = True
                                    if not crc_ok:
                                    ...
                                    rx_success = True
                                    if args.verbose:
                                        print(f"  [RX] Packet {rx_pkt_idx} OK (attempt={rx_attempt}) - BER: {bit_diff/len(ref_bits):.2e}")
                            except Exception as e:
                                if args.verbose:
