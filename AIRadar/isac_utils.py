...
C0 = 299_792_458.0  # speed of light
def _build_otfs_channel_from_gts(gts, sp: SystemParams, otfs_par, base_amp: float = 1000.0):
    """
...
def otfs_torch_full_radar(
    ...
    taps = _build_otfs_channel_from_gts(gts, sp, otfs_par, base_amp=base_amp)
...
class RadarSimDataset(torch.utils.data.Dataset):
    ...
    def simulate_to_disk(
        ...
        def _visualize_sample(
            ...
            # Scatter of channel scatterers (range vs v_r)
            # viz_channel_scatterers expects keys: R_targets, vr_targets, amp_targets
            # but extra_f has: R, vr, amp
            extra_chan_viz = {
                "R_targets": extra_f.get("R"),
                "vr_targets": extra_f.get("vr"),
                "amp_targets": extra_f.get("amp"),
            }
            viz_channel_scatterers(
                extra_chan=extra_chan_viz,  # FMCW extras carry R / vr / amp
                out_path=vis_dir / f"sample_{base_name}_channel_fmcw.png",
            )
        ...
        def _simulate_one_sample(idx: int, split: str):
            ...
            # 3) FMCW simulation
            # rd_f_db: (M, N//2) float32, Range-Doppler map in dB
            # extra_f: dict with IQ, RD complex, H_gt, axes, scatterer info
            rd_f_db, extra_f = fmcw_torch(
                pts,
                its,
                vels,
            ...
            #     return_extra=True,
            # )
            # dd_o_db: (M, N) float32, Delay-Doppler map in dB
            # extra_o: dict with H_est, H_gt, taps, axes, time signals
            dd_o_db, extra_o = otfs_torch_full_radar(
                ...
                return_extra=True,
                mti_order=1,
                base_amp=1000.0,  # Match FMCW amplitude scaling
            )
            # extra_o contains:
