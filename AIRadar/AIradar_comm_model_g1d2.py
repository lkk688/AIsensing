"""
AIradar_comm_model_g1d.py

Joint Radar+Comm Deep Model (no attention), generalized across configs.

Key features:
- Many-to-many training across multiple TRADITIONAL configs.
- Reuses previously generated datasets via joint_dump.npy to save time:
    * If joint_dump.npy exists and has >= N samples, we just load it.
    * Otherwise we call AIRadar_Comm_Dataset to generate more, then load.
- Config- and SNR-conditioned CNN model:
    * Config ID embedding
    * SNR MLP
    * Condition vector concatenated as channels to both radar and comm branches.
- Detailed evaluation:
    * Radar: CFAR vs Deep F1/precision/recall, range/velocity errors.
    * Comm: baseline BER vs Deep SER.
    * SNR sweep plots and radar ROC-like curves.
    * Per-sample 2D/3D RDM plots + comm constellation & eye diagrams.

Usage example:
    python AIradar_comm_model_g1d.py --mode train \
...
# ----------------------------------------------------------------------
# IMPORTANT: change "AIradar_comm_dataset_g1b" if your module name differs.
...
class RadarCommDumpDataset(Dataset):
    ...
    def __init__(self, config_name: str, num_samples: int, save_path: str):
        ...
        # Original AIRadar_Comm_Dataset uses:
        #   Nc = self.radar_Nc (64)
        #   Ns = int(self.radar_fs * self.radar_T)
        #   r_res = (c * fs) / (2 * slope * Ns)
        #   v_res = lambda_c / (2 * Nc * T)
...
class RadarCommDeepDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ---------- Radar inputs ----------
        # Ensure fresh, contiguous numpy array
...
# ----------------------------------------------------------------------
# Model definition: conv + residual + conditioning (no attention)
# ----------------------------------------------------------------------
...
class CommBranch(nn.Module):
    def __init__(self, in_ch=2, base_ch=48, num_blocks=4, max_mod_order=64):
        ...
        self.head = nn.Sequential(
            ...
            nn.BatchNorm2d(base_ch),
            nn.SiLU(inplace=True),
            nn.Conv2d(base_ch, max_mod_order, kernel_size=1),
        )
...
class JointRadarCommNet(nn.Module):
    ...
    Joint model with config+SNR conditioning:
      - config_id -> embedding
      - snr_db    -> MLP
      -> combined cond vector -> appended as channels to both branches.
    """
...
def extract_meta_tensors(meta, device):
    """
    DataLoader default_collate turns list-of-dicts into a single dict-of-tensors:
      meta = {
...
def main():
    ...
    parser.add_argument("--data_root", type=str,
                        default="data/AIradar_comm_model_g1d")
    ...
    parser.add_argument("--out_dir", type=str, default="data/AIradar_comm_model_g1d")
...
if __name__ == "__main__":
    main()
#Radar has very good generalization, but communication is not
"""
=== Running full evaluation on configs: ['AUTOMOTIVE_TRADITIONAL', 'Automotive_77GHz_LongRange', 'CN0566_TRADITIONAL', 'XBand_10GHz_MediumRange'] ===

[Dataset] No dump found for AUTOMOTIVE_TRADITIONAL split=test, generating 100 samples...
Generating 100 samples in TRADITIONAL mode...
Config: 64-QAM | Channel: multipath
100%|█████████████████████████████████████████████| 100/100 [00:59<00:00,  1.67it/s]
=== Radar Metrics (Classical CFAR) ===
Targets: 197
TP=197 FP=27 FN=0
Precision=0.8795 Recall=1.0000 F1=0.9359
Mean Range Error=0.025 m
Mean Velocity Error=0.117 m/s

=== Radar Metrics (Deep Model) ===
Targets: 197
TP=197 FP=0 FN=0
Precision=1.0000 Recall=1.0000 F1=1.0000
Mean Range Error=0.025 m
Mean Velocity Error=0.118 m/s

=== Communication Metrics ===
Baseline Mean BER=1.13250e-01
Deep Model SER≈BER=9.80179e-01
[Dataset] No dump found for Automotive_77GHz_LongRange split=test, generating 100 samples...
Generating 100 samples in TRADITIONAL mode...
Config: 4-QAM | Channel: multipath
100%|█████████████████████████████████████████████| 100/100 [00:29<00:00,  3.38it/s]
=== Radar Metrics (Classical CFAR) ===
Targets: 198
TP=198 FP=3 FN=0
Precision=0.9851 Recall=1.0000 F1=0.9925
Mean Range Error=0.025 m
Mean Velocity Error=0.208 m/s
...
Precision=1.0000 Recall=1.0000 F1=1.0000
Mean Range Error=0.025 m
Mean Velocity Error=0.209 m/s

=== Communication Metrics ===
Baseline Mean BER=3.24986e-03
Deep Model SER≈BER=7.69078e-01
[Dataset] No dump found for CN0566_TRADITIONAL split=test, generating 100 samples...
Generating 100 samples in TRADITIONAL mode...
Config: 16-QAM | Channel: multipath
100%|█████████████████████████████████████████████| 100/100 [00:12<00:00,  7.94it/s]
=== Radar Metrics (Classical CFAR) ===
Targets: 208
TP=208 FP=33 FN=0
Precision=0.8631 Recall=1.0000 F1=0.9265
Mean Range Error=0.072 m
Mean Velocity Error=0.116 m/s

=== Radar Metrics (Deep Model) ===
Targets: 208
TP=205 FP=7 FN=3
Precision=0.9670 Recall=0.9856 F1=0.9762
Mean Range Error=0.074 m
Mean Velocity Error=0.128 m/s

=== Communication Metrics ===
Baseline Mean BER=4.96652e-03
Deep Model SER≈BER=4.74632e-01
[Dataset] No dump found for XBand_10GHz_MediumRange split=test, generating 100 samples...
Generating 100 samples in TRADITIONAL mode...
Config: 16-QAM | Channel: multipath
...
TP=204 FP=0 FN=0
Precision=1.0000 Recall=1.0000 F1=1.0000
Mean Range Error=0.038 m
Mean Velocity Error=0.359 m/s
...
Precision=1.0000 Recall=1.0000 F1=1.0000
Mean Range Error=0.038 m
Mean Velocity Error=0.360 m/s

=== Communication Metrics ===
Baseline Mean BER=4.17411e-03
Deep Model SER≈BER=4.77679e-03
"""