...
def radar_metrics_otfs(targets, detections, range_thresh=10.0, vel_thresh=25.0):
    """Compute radar detection metrics for OTFS with relaxed velocity matching.
    
    OTFS has coarse velocity resolution (~17.86 m/s per bin), so we:
    1. Use separate range and velocity thresholds (not euclidean)
    2. Use larger velocity threshold to account for quantization
    
    Args:
        targets: List of target dicts with 'range' and 'velocity'
        detections: List of detection dicts with 'range_m' and 'velocity_mps'
        range_thresh: Max range error in meters (default 10m)
        vel_thresh: Max velocity error in m/s (default 25 m/s ~ 1.5 bins)
    ...
    for det in detections:
        ...
        for i, t in enumerate(targets):
            ...
            # Use separate thresholds for range and velocity
            if dr < range_thresh and dv < vel_thresh:
                tp += 1
...
@torch.no_grad()
def _evaluate_radar_otfs(model, device, config_name, snr_list, num_samples, save_path):
    """Evaluate OTFS radar with relaxed velocity matching.
    
    OTFS has coarse velocity resolution (~17.86 m/s per bin), so we use:
    - Range threshold: 10m (vs 3m euclidean for FMCW)
    - Velocity threshold: 25 m/s (allows ~1.5 bin error)
    """
    os.makedirs(save_path, exist_ok=True)
    model.eval()
    ...
    for snr_db in snr_list:
        ...
        config_tensor = torch.tensor([
            config.get('fc', 77e9) / 1e9,
            config.get('B', 4e9) / 1e9,
            config.get('N_delay', 64) / 64.0,
            config.get('mod_order', 4) / 64.0,
            snr_db / 30.0,
            3.75,  # OTFS range resolution
            config.get('R_max', 100) / 100.0,
            50 / 50.0,
        ], dtype=torch.float32).unsqueeze(0).to(device)
        ...
        for sample in ds:
            ...
            targets = sample.get('target_info', {}).get('targets', [])
            r_axis = np.array(sample.get('range_axis', np.linspace(0, 100, rdm.shape[2])))
            v_axis = np.array(sample.get('velocity_axis', np.linspace(-50, 50, rdm.shape[1])))
            
            # DL detection with lower threshold for OTFS
            radar_logits = model(radar_in, config_tensor)
            probs = torch.sigmoid(radar_logits)[0, 0].cpu().numpy()
            dl_dets = postprocess_radar(probs, r_axis, v_axis, prob_thresh=0.5)  # Lower threshold for OTFS
            
            # Use OTFS-aware metrics with relaxed velocity matching
            dl_m = radar_metrics_otfs(targets, dl_dets, range_thresh=10.0, vel_thresh=25.0)
            dl_total['tp'] += dl_m['tp']
            dl_total['fp'] += dl_m['fp']
            dl_total['fn'] += dl_m['fn']
            
            # CFAR with same relaxed matching
            cfar_dets = sample.get('cfar_detections', [])
            cfar_dets_formatted = [{'range_m': d.get('range_m', d.get('range', 0)), 
                                    'velocity_mps': d.get('velocity_mps', d.get('velocity', 0))} 
                                   for d in cfar_dets]
            cfar_m = radar_metrics_otfs(targets, cfar_dets_formatted, range_thresh=10.0, vel_thresh=25.0)
            cfar_total['tp'] += cfar_m['tp']
            cfar_total['fp'] += cfar_m['fp']
            cfar_total['fn'] += cfar_m['fn']
...
def evaluate_radar_type_comparison(device, out_dir, snr_list=[5, 10, 15, 20, 25, 30], num_samples=15):
    ...
    Uses separate matching thresholds for FMCW (euclidean) vs OTFS (range-only with relaxed velocity)
    because OTFS has coarse velocity resolution (~17.86 m/s per bin).
    ...
    for radar_type in ['FMCW', 'OTFS']:
        ...
        # Choose metrics function based on radar type
        is_otfs = (radar_type == 'OTFS')
        
        if is_otfs:
            # Use custom OTFS evaluation with relaxed velocity matching
            print(f"  [Using OTFS-aware matching: range<10m, velocity<25m/s]")
            type_results = _evaluate_radar_otfs(radar_model, device, config_name, snr_list, num_samples, save_path)
        else:
            # Use standard evaluation for FMCW
            type_results = evaluate_radar_by_snr(
                radar_model, device, config_name,
                snr_list=snr_list, num_samples=num_samples,
                save_path=save_path
            )
        
        type_results['config'] = config_name  # Store config name for report
        results[radar_type] = type_results
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='data/AIradar_comm_model_g3b')
