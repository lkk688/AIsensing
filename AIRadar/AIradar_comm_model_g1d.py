...
- Reuses previously generated datasets via joint_dump.npy to save time:
    ...
    * Otherwise we call AIRadar_Comm_Dataset to generate more and then load.
...
Usage example:
    python AIradar_comm_model_g1c.py --mode train \
...
# ----------------------------------------------------------------------
# Losses & multi-config train/eval loops
# ----------------------------------------------------------------------
...
def run_full_evaluation(model, deep_ds, base_ds, cfg, device, out_dir,
    ...
    for idx in range(len(deep_ds)):
        ...
        model.eval()
        radar_logits, comm_logits = model(
            radar_in_b, comm_in_b, config_ids=cfg_id, snr_db=snr_tensor
        )
