...
def apply_radar_channel_and_noise(tx_signal, snr_db, physical_targets, fs_hz):
    ...
    if signal_power == 0: noise_power = 1e-20 # Avoid div by zero if no signal
...
def main():
    ...
    ddm_otfs_mag_db = 10 * np.log10(np.abs(ddm_otfs_shifted))
    ...
    for snr_db in SNR_DB_LIST:
        ...
        # This is the standard, reliable way to do a BER sim
        # 1. 2D Convolution in DD domain: Y = H ** X
        ...
        if signal_power == 0: signal_power = 1e-20 # avoid div by zero
        snr_linear = 10**(snr_db / 10)
        ...
        # 3. Equalize: X_est = IFFT2( FFT2(Y) / FFT2(H) )
        Y_tf = np.fft.fft2(rx_dd_noisy)
        H_tf_reg = np.fft.fft2(H_ideal_dd_comm)
        H_tf_reg[np.abs(H_tf_reg) < 1e-6] = 1e-6 # Regularization
