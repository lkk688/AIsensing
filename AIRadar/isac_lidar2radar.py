...
Highlights
----------
- NEW: Added `detect_targets_from_rd` function for peak detection (CFAR-like).
- NEW: `viz_rd` now plots Detections (cyan 'o') vs. Ground Truth (red 'x').
- NEW: `run_dataset_demo` now runs the full detection pipeline and prints
         a comparison of detected targets vs. ground truth.
- NEW: Ground Truth markers added to 3D RD plot.
- NEW: `rcs.png` plot logic is corrected.
- NEW: `bev_3d` plot is "zoomed in" to the scene's max range.
- NEW: Fixed matplotlib `AttributeError` in Arrow3D class.
- MTI filter (2-pulse canceler) removes static clutter.
...
@dataclass
class OTFSParams:
    """OTFS ISAC Parameters"""
    fc: float = 77e9            # Carrier [Hz]
    fs: float = 10e6            # Bandwidth [Hz]
    M_doppler: int = 64         # Doppler bins (subcarriers)
    N_delay: int = 64           # Delay bins (time slots)
    mod_order: int = 16         # QAM order (e.g., 16-QAM)
    noise_figure_dB: float = 5.0
    
    def lambda_m(self): return C0 / self.fc
    def delta_f(self): return self.fs / self.M_doppler
    def T_sym(self): return 1.0 / self.fs # Time per delay bin (approx 1/B)
    def total_duration(self): return self.N_delay * (1.0 / self.delta_f())
    # OTFS sensing resolution limits
    def max_range(self): return (C0 * self.N_delay * self.T_sym()) / 2.0
    def max_velocity(self): return (self.lambda_m() * self.delta_f() * self.M_doppler) / 4.0
...
# -----------------------------
# NEW: FMCW Target Detection
# -----------------------------
def detect_targets_from_rd(RD_dB, params: RadarParams, noise_floor_db=-100, peak_thresh_db=15):
    """
    Detects targets (peaks) in the Range-Doppler map.
    
    - Masks the central DC clutter ridge.
    - Applies a threshold above the noise floor.
    - Finds local peaks in the thresholded map.
    
    Returns:
        List of dictionaries: [{'range': r, 'velocity': v}, ...]
    """
    if ndi is None:
        print("Error: Scipy is not installed. Cannot perform target detection.")
        return []

    M, Ns = RD_dB.shape
    
    # 1. Mask out the central DC clutter ridge (and nearby bins)
    dc_bin_index = M // 2
    clutter_bins = 3 # Number of bins to mask (1 center + 1 on each side)
    clutter_start = max(0, dc_bin_index - clutter_bins // 2)
    clutter_end = min(M, dc_bin_index + (clutter_bins // 2) + 1)
    
    # Create a mask that is True *except* for the clutter bins
    detection_mask = np.ones_like(RD_dB, dtype=bool)
    detection_mask[clutter_start:clutter_end, :] = False

    # 2. Determine threshold
    # A simple threshold: X dB above the specified noise floor
    detection_threshold = noise_floor_db + peak_thresh_db # e.g., -100 + 15 = -85 dB

    # 3. Find all points above threshold *and* outside the clutter mask
    binary_mask = (RD_dB > detection_threshold) & detection_mask
    ...
    # 5. Find the peak coordinate (max value) within each cluster
    peak_coords_bins = ndi.maximum_position(RD_dB, labels=labeled_mask, index=np.arange(1, num_clusters + 1))
    
    if not isinstance(peak_coords_bins, list):
        peak_coords_bins = [peak_coords_bins]

    # 6. Convert bin coordinates to (range, velocity)
    R_unamb = params.unambiguous_range()
    v_unamb = params.unambiguous_doppler_vel()
    range_res = R_unamb / Ns
    doppler_res = v_unamb / M
    doppler_center_bin = M // 2
    
    detections = []
    for (d_bin, r_bin) in peak_coords_bins:
        range_m = r_bin * range_res
        velocity_ms = (d_bin - doppler_center_bin) * doppler_res
        detections.append({'range': range_m, 'velocity': velocity_ms})
...
def viz_rd(RD_dB, out_png, params: RadarParams, target_gt_list=None, detections=None, sensor_origin=np.array([0.0,0.0,0.0])):
    M, Ns = RD_dB.shape
    ...
    if detections is not None and len(detections) > 0:
        det_ranges = [d['range'] for d in detections]
        det_vels = [d['velocity'] for d in detections]
        
        plt.plot(det_ranges, det_vels, 'o', markerfacecolor='none', 
                 markeredgecolor='cyan', markersize=12, markeredgewidth=2, label='Detections')

    if (gt_coords and len(gt_coords) > 0) or (detections and len(detections) > 0):
        plt.legend()
...
def viz_bev_3d(out_png, pts_xyz, target_gt_list, params: RadarParams, lidar_config: dict, sensor_origin=None):
    ...
    # <--- FIX: Removed invalid 'arrow_length_ratio' argument --->
    ...
    for gt in target_gt_list:
        ...
        if range_m > 0:
            ...
            if abs(radial_vel_scalar) > 0.1:
                ...
                # <--- FIX: Removed invalid 'arrow_length_ratio' and added color --->
    ...
    # <--- FIX: "Zoom in" plot to lidar_config max range --->
...
def run_dataset_demo(outdir):
    ...
    if ndi is None:
        print("ERROR: 'scipy' is required for the demo to run detection. Please install it.")
        return
    ...
    target_config = {
        ...
        "high_vel_x": (7.0, 15.0), # <--- Force high *radial* velocity
        "low_vel_x": (-1.5, 1.5), # <--- Force low *radial* velocity (in MTI blind spot)
        "vel_y": (-2.0, 2.0),
        "vel_z": (0.0, 0.0)
    }
    ...
    for i in range(3):
        ...
        for j, gt in enumerate(gt_list):
            ...
            is_in_blind_spot = abs(rad_vel_ms) < 2.0 
        ...
        # <--- NEW: Run detection and print results --->
        detections = detect_targets_from_rd(RD_dB, sample['params'], noise_floor_db=-110, peak_thresh_db=20)
        print(f"--- Detections for Sample {i} ({len(detections)} targets) ---")
        for j, d in enumerate(detections):
            print(f"  Detection {j+1}: Range: {d['range']:.2f} m, Velocity: {d['velocity']:.2f} m/s")
        print("----------------------------------")
        
        print(f"Saving visualizations to: {viz_dir}/sample_{i:02d}_*")
        
        viz_rd(RD_dB, os.path.join(viz_dir, f"sample_{i:02d}_rd_2d.png"), 
               sample['params'], target_gt_list=gt_list, detections=detections, sensor_origin=sensor_origin)
    ...
    print("Check 'sample_XX_rd_2d.png' to compare Detections (cyan 'o') vs. Ground Truth (red 'x').")

def run_otfs_demo(outdir):
    os.makedirs(outdir, exist_ok=True)
...
def process_real_dataset(adapter, params: RadarParams, outdir, viz_every=20):
    ...
    if ndi is None:
        print("ERROR: 'scipy' is required for processing. Please install it.")
        return
    ...
    for i in range(len(dataset)):
        ...
        if (i % viz_every) == 0:
            ...
            detections = detect_targets_from_rd(RD_dB, params, noise_floor_db=-110, peak_thresh_db=20)
            
            viz_rd(RD_dB, os.path.join(viz_dir, f"rd_2d_{i:06d}.png"), params, 
                   target_gt_list=gt_list, detections=detections, sensor_origin=sensor_origin)
                   
            viz_rd_3d(RD_dB, os.path.join(viz_dir, f"rd_3d_{i:06d}.png"), params, 
                      target_gt_list=gt_list, sensor_origin=sensor_origin)
