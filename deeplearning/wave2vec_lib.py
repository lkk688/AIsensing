...
from transformers.models.wav2vec2.modeling_wav2vec2 import Wav2Vec2PositionalConvEmbedding, Wav2Vec2EncoderStableLayerNorm
...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        # for the decoder
        is_cross_attention = key_value_states is not None #False

        bsz, tgt_len, _ = hidden_states.size() #[1, 270, 768]

        # get query proj
        query_states = self.q_proj(hidden_states) * self.scaling #768->768 [1, 270, 768]
        # get key, value proj
        ...
        if (
            ...
            # self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz) #[1, 270, 768]->[1, 12, 270, 64]
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz) #->[1, 12, 270, 64]
        ...
        query_states = self._shape(query_states, tgt_len, bsz).view(*proj_shape) #[12, 270, 64]
        key_states = key_states.reshape(*proj_shape) #[12, 270, 64]
        value_states = value_states.reshape(*proj_shape) #[12, 270, 64]

        src_len = key_states.size(1) #270
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2)) #[12, 270, 270]
        ...
        attn_output = torch.bmm(attn_probs, value_states) #[12, 270, 64]
        ...
        attn_output = self.out_proj(attn_output) #[1, 270, 768]
...
class Wav2Vec2EncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        attn_residual = hidden_states #[1, 270, 768]
        hidden_states, attn_weights, _ = self.attention(
            hidden_states, attention_mask=attention_mask, output_attentions=output_attentions
        )
        hidden_states = self.dropout(hidden_states) #[1, 270, 768]
...
class Wav2Vec2Encoder(nn.Module):
    ...
    def forward(
        ...
        hidden_states = hidden_states + position_embeddings #[1, 270, 768]
        hidden_states = self.layer_norm(hidden_states)
        hidden_states = self.dropout(hidden_states)
