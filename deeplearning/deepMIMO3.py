...
from sionna_tf import ZFPrecoder, LMMSEEqualizer, Demapper, RemoveNulledSubcarriers, KroneckerPilotPattern
from channel import ApplyOFDMChannel, LSChannelEstimator
from ldpc.encoding import LDPC5GEncoder
from ldpc.decoding import LDPC5GDecoder

from sionna_lib import qam, pam, show, plotcomplex, ebnodb2no
...
from sionna_tf import ResourceGrid
...
if __name__ == '__main__':
    ...
    from sionna_lib import MyDemapper
    mydemapper = MyDemapper("app", constellation_type="qam", num_bits_per_symbol=num_bits_per_symbol)
    #llr = mydemapper.demap([y, no]) #[64, 1, 1, 14, 304]
    llr = mydemapper.demap([x_rg, no]) #[64, 1, 1, 14, 304]
    
    demapper = Demapper("app", "qam", num_bits_per_symbol)
    llr = demapper([x_rg, no])  #[64, 1, 1, 14, 304]
    #llr = demapper([x_hat, no_eff])  #[64, 1, 1, 3648] 912*4

    # Reshape the array by collapsing the last two dimensions
    llr_np=llr.numpy()
    llr_np = llr_np.reshape(llr_np.shape[:-2] + (-1,)) #(64, 1, 1, 4256)

    b_hat = decoder(llr_np) #[64, 1, 1, 2128]
    #b_hat = hard_decisions(llr, np.int32) #(64, 1024) 0,1 [64, 1, 1, 14, 304] 2128
    #b=b.numpy()
    b_hat=b_hat.numpy()
    errors = (b.flatten() != b_hat.flatten()).sum()
    N = len(b.flatten())
    BER = 1.0 * errors / N
    print(BER)
    ...
    b_hat = hard_decisions(llr, np.int32) #(64, 1024) 0,1
    b=b.numpy()
    b_hat=b_hat.numpy()
    errors = (b != b_hat).sum()
    N = len(b.flatten())
